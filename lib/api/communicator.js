...
export class Communicator {
  ...
  initWebWorkerListeners() {
    self.addEventListener('message', (event) => {
      if (event.data.type === EVENTS.MIMIC_SET_DATA) {
        ...
        this.mocks = event.data.payload.map((request) => new MockedRequest(request));
      }
    });
  }
  ...
  waitForInit() {
    return new Promise((resolve, reject) => {
      ...
      if (!this.isRemote && !this.isWebWorker) {
        Emitter.on(EVENTS.STORAGE_READY, () => {
          this.initialized = true;
          resolve();
        });
    ...
    return this.waitForInit()
      .then(() => {
        return new Promise((resolve) => {
          if (!this.isWebWorker) {
            resolve(MockedRequests.getCurrentMockedRequests());
          }
  ...
  setCapturedRequestResponse(id, response, startTime) {
    const jsonResponse = {
    ...
    Requests.setResponse(id, response, startTime);
  }
