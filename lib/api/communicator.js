...
export class Communicator {
  ...
  initWebWorkerListeners() {
    self.addEventListener('message', (event) => {
      if (event.data.type === EVENTS.BDSM_SET_DATA) {
  ...
  initRemoteListeners(options) {
    ...
    this.socket.on('connect', () => {
      this.socket.emit('bdsm-message', { type: EVENTS.GET_MOCKED_REQUESTS });
    });

    const channel = this.socket.subscribe('bdsm-message');

    channel.watch((message) => {
      if (message.type === EVENTS.BDSM_SET_DATA) {
        Scenarios.setScenarios(message.payload);
        this.initialized = true;
      }
    });

    Emitter.on(EVENTS.BDSM_WEB_WORKER_CAPTURE, (payload) => {
  ...
  waitForInit() {
    return new Promise((resolve, reject) => {
      ...
      let channel = this.socket.subscribe('bdsm-message');
  ...
  captureRequest(request, response) {
    ...
    if (this.isWebWorker) {
      self.postMessage({ type: WORKER_MESSAGE_TYPES.BDSM_CAPTURE_REQUEST, payload: {
  ...
  emit(event) {
    if (this.isWebWorker) {
      self.postMessage({ type: WORKER_MESSAGE_TYPES.BDSM_EMIT_EVENT, payload: event });
      return;
    }
