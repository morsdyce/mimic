...
export class Mock {
  ...
  constructor({ id, active = true, method, url, headers, params, response, name, origin, groupId }) {
    assign(this, {
      ...
      params,
      response: isEmpty(response) ? cloneDeep(DEFAULT_RESPONSE) : response,
      name,
      origin,
      groupId
    });

    if (!get(this, 'response.headers[content-type]')) {
      set(this, 'response.headers[content-type]', headers['content-type']);
    }
  }
  ...
  matches(request) {
    ...
    const methodMatches = normalizedRequest.method.toLowerCase() === this.method.toLowerCase();

    return methodMatches &&
      this.matchParams(normalizedRequest.params) &&
