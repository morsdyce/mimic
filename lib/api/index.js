...
class PublicAPI {
  ...
  connect(options) {
    ...
    socket.on('connect', () => {
      ...
      socket.emit('bdsm-message', { type: 'BDSM_SET_DATA', payload: API.mockedRequests });
      API.on(EVENTS.STORAGE_PERSIST, () => socket.emit('bdsm-message', { type: 'BDSM_SET_DATA', payload: API.mockedRequests }));
    });
    ...
    channel.watch(function (message) {
      ...
      if (message.type === 'GET_MOCKED_REQUESTS') {
        console.log('request for mocks recieved', API.mockedRequests);
        socket.emit('bdsm-message', { type: 'BDSM_SET_DATA', payload: API.mockedRequests });
      }
    });
