...
if (!get(global, 'isReactNative') && !get(global, 'isNativeScript')) {
...
class PublicAPI {
  ...
  import(data = '{}', options) {
    return new Promise((resolve, reject) => {
      if (interceptor) {
        return interceptor.communicator.waitForInit()
          .then(() => {
            try {
              const dataTree = JSON.parse(data);
              let formattedTree;

              if (options && options.external_import) {
                formattedTree = new Transformer(options.external_import).toMimic(dataTree);
              } else {
                formattedTree = migrateData(dataTree);
              }

              Mocks.mergeMocks(formattedTree.mocks || [], options);
              Groups.mergeGroups(formattedTree.groups || []);
