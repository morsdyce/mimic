...
class PublicAPI {
  ...
  get mocks() {
    return Mocks.getMocks();
  }

  getMock(mockId) {
    return Mocks.getMock(mockId);
  }

  get activeMocks() {
    return Mocks.getActiveMocks();
  }
  ...
  export(prettify) {
    const mocks = Mocks.mocks.map((mock) => mock.export());
  ...
  exportMock(mockId, prettify) {
    const mocks = Mocks.exportMock(mockId);
  ...
  exportMocks(mockIds, prettify) {
    const mocks = Mocks.exportMocks(mockIds);
  ...
  updateMock(mockRequestId, request) {
    Mocks.updateMock(mockRequestId, request);
