...
class PublicAPI {
  ...
  get mocks() {
    return MockedRequests.getMocks();
  }

  getMock(mockId) {
    return MockedRequests.getMock(mockId);
  }

  get activeMocks() {
    return MockedRequests.getActiveMocks();
  }
  ...
  import(data = '{}') {
    ...
    MockedRequests.mergeMocks(dataTree.mocks);
  ...
  updateMock(mockRequestId, request) {
    MockedRequests.updateMock(mockRequestId, request);
    Emitter.emit(EVENTS.MOCKED_REQUEST_CHANGE);
  }

  removeMock(mocktId) {
    MockedRequests.removeMock(mocktId);
  ...
  toggleMock(mocktId) {
    MockedRequests.toggleMock(mocktId);
    Emitter.emit(EVENTS.MOCKED_REQUEST_CHANGE);
  }
