  ...
  mergeMocks(mocks, options = {}) {
    for (const mock of mocks) {
      ...
      if (isObject(mock.response.body)) {
        mock.response.body = JSON.stringify(mock.response.body).replace(/\s/g, '');
      }
      ...
      if (existingMock.response.body) {
        existingMock.response.body = existingMock.response.body.replace(/\s/g, '');
      }
      const isMockEqual = JSON.stringify(mock) === JSON.stringify(existingMock);
