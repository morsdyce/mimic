...
export function bootstrapWorker(worker, API) {
  worker.addEventListener('message', (event) => {
    ...
    if (messageType === 'BDSM_CAPTURE_REQUEST') {
      ...
      if (API.mode === 'remote') {
        Emitter.emit('BDSM_WEB_WORKER_CAPTURE', { request, response });
      }
    ...
    if (messageType === 'BDSM_EMIT_EVENT') {
      event.stopImmediatePropagation();
      Emitter.emit(event.data.payload);
    }
  });

  worker.postMessage({ type: 'BDSM_SET_DATA', payload: API.mockedRequests });
  API.on(EVENTS.STORAGE_PERSIST, () => worker.postMessage({ type: 'BDSM_SET_DATA', payload: API.mockedRequests }));
}
