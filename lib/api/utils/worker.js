...
export function bootstrapWorker(worker, API) {
  ...
  worker.postMessage({ type: EVENTS.MIMIC_SET_DATA, payload: API.mockedRequests });
  API.on(EVENTS.STORAGE_PERSIST, () => worker.postMessage({ type: EVENTS.MIMIC_SET_DATA, payload: API.mockedRequests }));
}
