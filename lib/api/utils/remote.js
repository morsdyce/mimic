...
export function registerRemoteHandlers(socket, API) {
  if (API.mode === 'remote') {
    return;
  }
  ...
  channel.watch(function (message) {
    ...
    if (message.type === EVENTS.SET_REQUEST_MOCK) {
      Requests.setMock(message.payload.requestId, message.payload.mock);
    }
