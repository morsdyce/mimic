...
import * as Communicator from 'api/communicator';
...
class XHRInterceptor {
  ...
  constructor() {
    xhook.before((request, responder) => {
      ...
      Communicator.getCurrentMockedRequests()
        .then((mockedRequests) => {
  ...
  captureRequest(request, response = {}) {
    ...
    if (!(request instanceof Request)) {
      Communicator.captureRequest(request, response);
      Communicator.emit(EVENTS.REQUEST_CAPTURED);
      return;
    }
    ...
    Promise.all([
      ...
      Communicator.captureRequest(capturedRequest, capturedResponse);
      Communicator.emit(EVENTS.REQUEST_CAPTURED);
    });
  }
