...
class XHRInterceptor {
          ...
          this.getRequestDetails(request).then(({ url, method, body }) => {
            for (let mock of mocks) {
              if (mock.matches({ url, params: body, method })) {
                this.communicator.setRequestMock(request.id, mock);
  ...
  updateCapturedRequest(request, response) {
    if (!(request instanceof Request)) {
      this.communicator.setCapturedRequestResponse(request, response);
    ...
    clonedResponse
      ...
      .then((body) => {
        ...
        this.communicator.setCapturedRequestResponse(request, capturedResponse);
        this.communicator.emit(EVENTS.RESPONSE_RECEIVED, { requestId: request.id });
      });
  }
  ...
  buildResponse(responder, responseRule, request) {
    ...
    if (response.headers['content-type'].indexOf('application/json') !== -1) {
      try {
        response.data = JSON.parse(responseRule.body);
      } catch (error) {
