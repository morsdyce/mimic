...
export class Groups {
  ...
  mergeGroups(groups, mocks, options = {}) {
    for (const group of groups) {
      const existingGroup = find(this.all, { id: group.id });

      if (existingGroup && options.mode === 'clear') {
        Object.assign(existingGroup, group);
      } else if (existingGroup && options.mode === 'append') {
        const duplicatedGroup = new Group(omit(group, ['id']));

        mocks
          .filter((mock) => mock.groupId === group.id)
          .forEach((mock) => {
            Mocks.updateMock(mock.id, { ...omit(mock, ['id']), groupId: duplicatedGroup.id });
          });

        this.all.push(duplicatedGroup);
      } else {
  ...
  duplicateGroup(groupId) {
    ...
    const duplicatedGroup = this.addGroup({
      ...omit(group, ['id']),
      name: `${group.name} Copy`
    });

    group.mocks.forEach((mock) => Mocks.duplicateMock(mock.id, { groupId: duplicatedGroup.id }));
