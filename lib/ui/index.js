...
import LatestRequestNotifications from 'ui/components/latest-request-notifications';
...
function initUIToggle() {
  ...
  ReactDOM.render(<UIToggle onToggle={ toggleUI }/>, container);
  document.body.appendChild(container);
}

function initRequestNotificationsArea(API) {
  const container = document.createElement('div');
  ReactDOM.render(<LatestRequestNotifications />, container);
  document.body.appendChild(container);

  API.on(EVENTS.REQUEST_SENT, (event) => console.log('REQUEST_SENT', event));
  API.on(EVENTS.RESPONSE_RECEIVED, (event) => console.log('RESPONSE_RECEIVED', event));
}
...
function APIProxy(API) {
  ...
  API.on(EVENTS.REQUEST_SENT, () => sendMessage({ type: EVENTS.REQUEST_SENT }));
...
export default function bootstrapUI(API) {
  ...
  ReactDOM.render(
    ...
    root, () => {
      ...
      APIProxy(API);
      initUIToggle();
      initRequestNotificationsArea(API);
    });
