    ...
    return (
      <Container fadingOut={ this.state.fadingOut }
        ...
        <Control onClick={ this.onEdit }>
          ...
          { mock && mock.active && <Icon style={{ marginRight: 6 }} src="mocked"/> }
          { mock && !mock.active && <Icon style={{ marginRight: 6 }} src="unmocked"/> }

          <MethodLabel mocked={ mock && mock.active }
                       failed={ status >= 400 }>
            { this.state.latestRequest.method }
          </MethodLabel>
          <URL mocked={ mock && mock.active }
               status={ status } >
            { this.state.latestRequest.url }
          </URL>

          { status && <Status>{ status }</Status> }

          { !this.state.latestRequest.response.status && <Icon src="spin"/> }
