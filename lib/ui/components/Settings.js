...
class Settings extends React.Component {
  ...
  importURL = () => {
    API.importUrl(this.urlInput.value, (status) => {
      if (status.success) {
        ...
        this.setState({ importFailed: true, error: status.error });
      }
    })
  };
  ...
  selectFile = (event) => {
    ...
    reader.onload = function handleFileUpload() {
      ...
      if (status.success) {
        self.setState({ importSuccess: true });
      } else {
        self.setState({ importFailed: true, error: status.error });
      }
    };
      ...
      <Frame>
        <Container>
          <Section>
            ...
            <Block>
              <Input type="text" defaultValue={ 'https://...' } innerRef={ (node) => this.urlInput = node }/>
            ...
            <HiddenFileInput
              ref={ (node) => { this.importFileInput = node; } }
              type="file"
              name="file"
