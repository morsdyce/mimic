...
export class Group extends React.Component {
  ...
  componentWillMount() {
    ...
    const group = find(MocksSidebarState.groups, { id });

    if (!group) {
      MocksSidebarState.setGroups([
  ...
  toggle = (event) => {
    ...
    const group = find(MocksSidebarState.groups, { id: this.props.id });
  ...
  updateGroup = (partialUpdate) => {
    const id = this.props.id;
    const groups = MocksSidebarState.groups.map((group) => {
      if (group.id === id) {
        return {
          id,
          ...partialUpdate
