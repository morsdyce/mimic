...
class MocksSidebar extends React.Component {
  ...
  select = (item, isRightClick) => (event) => {
    if (isRightClick && UIState.selectedItems.length > 1) {
      return;
    }
    ...
    if (event.shiftKey && UIState.selectedItems.length > 0) {
      ...
      const selectedItems = list.filter((listItem) => UIState.selectedItems.indexOf(listItem) !== -1);
      ...
      return itemsBetween
        .filter((itemInRange) => UIState.selectedItems.indexOf(itemInRange) === -1)
      ...
      .map((group) => {
        ...
        return (
          <Group key={ group.id }
                 ...
                 name={ group.name }
                 active={ group.active }
                 isSelected={ !!find(UIState.selectedItems, { id: group.id }) }
                 mocks={ mocks }
                 searchTerm={ this.props.searchTerm }
