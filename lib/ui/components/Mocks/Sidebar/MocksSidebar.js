...
import Draggable from 'ui/components/common/Draggable';
...
class MocksSidebar extends React.PureComponent {
  ...
  getMocks() {
    return API.mocks
      ...
      .filter((mock) => includes(mock.url, this.props.searchTerm))
  ...
  getGroups() {
    return API.groups
      .filter((group) => {
        ...
        if (includes(group.name, this.props.searchTerm)) {
          return true;
        }

        return some(group.mocks, (mock) => includes(mock.url, this.props.searchTerm))
      })
      .map((group) => {
  ...
  render() {
    return (
      <Draggable
        style={{ position: 'relative', height: 'calc(100% - 26px)' }}
