...
class MocksSidebar extends React.PureComponent {
  ...
  onKeyDown = (event) => {
    if (event.keyCode === KEYCODES.BACKSPACE || event.keyCode === KEYCODES.DELETE) {
        ...
        const selectedMocksIds = this.props.selectedMocks
            .filter(mock => !includes(selectedGroupsIds, mock.groupId))
            .map(mock => mock.id);

        selectedGroupsIds.forEach(id => API.removeGroup(id));
        selectedMocksIds.forEach(id => API.removeMock(id));
    }
};
