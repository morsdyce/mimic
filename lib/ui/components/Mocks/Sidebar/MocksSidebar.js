...
class MocksSidebar extends React.Component {
  ...
  select = (item, isRightClick) => (event) => {
    if (isRightClick && MocksSidebarState.hasSelection) {
      return;
    }
    ...
    if (event.shiftKey && MocksSidebarState.selectedItems.length > 0) {
      ...
      const selectedItems = list.filter((listItem) => MocksSidebarState.selectedItems.indexOf(listItem) !== -1);
      ...
      return itemsBetween
        .filter((itemInRange) => MocksSidebarState.selectedItems.indexOf(itemInRange) === -1)
    ...
    return API.mocks
      ...
      .map((mock) => (
          <Mock key={ mock.id }
                ...
                isSelected={ !!find(MocksSidebarState.selectedMocks, { id: mock.id }) }
      ...
      .map((group) => {
        ...
        return (
          <Group key={ group.id }
                 ...
                 name={ group.name }
                 active={ group.active }
                 isSelected={ !!find(MocksSidebarState.selectedGroups, { id: group.id }) }
                 mocks={ mocks }
                 searchTerm={ this.props.searchTerm }
  ...
  openContextMenu = (event) => {
    ...
    MocksSidebarState.openMenu(event.clientX, event.clientY);
  };
  ...
  render() {
    return this.props.connectDropTarget(
      <div style={{ position: 'relative', height: '100%' }}>
        ...
        <SidebarContainer style={{ width: UIState.mocksSidebarWidth }}
