...
class QuickEdit extends React.Component {
  ...
  onSave() {
    ...
    const { delay, statusCode, headerPreset, responseBody } = this.state;
    const mockedRequest = _.find(this.props.API.mockedRequests, { id: this.props.selectedRequestId });

    const request = Object.assign({}, this.props.latestRequest, {
      response: {
        delay,
        statusCode,
    ...
    if (mockedRequest) {
      this.props.API.updateMockedRequest('default-scenario', selectedRequestId, request);
    } else {
