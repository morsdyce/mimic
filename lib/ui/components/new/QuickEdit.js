...
class QuickEdit extends React.Component {
  ...
  initRequest(props) {
    ...
    return {
      ...
      statusCode: _.get(mock, 'response.statusCode') || _.get(latestRequest, 'response.statusCode', 200),
      headerPreset: 'JSON',
  ...
  onSave() {
    ...
    const { delay, statusCode, headerPreset, responseBody, active } = this.state;
  ...
  render() {
    return (
      <div style={ containerStyle }>
        ...
        <div style={ sectionStyle }>
          <input type="checkbox" onChange={ this.onChangeEvent('active') }/>
          <CompactSelect
        ...
        <div style={ responseBodySectionStyle }>
          <input type="text"
                 onChange={ this.onChangeEvent('responseBody') }
