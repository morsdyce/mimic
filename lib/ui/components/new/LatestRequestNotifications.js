...
class LatestRequestNotifications extends React.Component {
  ...
  resetFadeoutTimer() {
    ...
    this.setState({ fadingOut: false }, () => {
      this.willFadeOut = setTimeout(() => this.setState({ fadingOut: true }, () => {
        this.willDisappear = setTimeout(() => this.setState({ latestRequest: null }), 3000);
      }), 3000);
    });
  }
  ...
  render() {
    ...
    const Wrapper = this.state.fadingOut ? FadingContainer : Container;

    return (
      <Wrapper onMouseEnter={ this.stopFadeoutTimer } onMouseLeave={ this.resetFadeoutTimer }>
