...
class LatestRequestNotifications extends React.Component {
  ...
  componentDidMount() {
    this.props.API.on(EVENTS.REQUEST_CAPTURED, ({ requestId }) => {
      this.setState({ latestRequest: this.props.API.getCapturedRequest(requestId) });
    });

    this.props.API.on(EVENTS.RESPONSE_RECEIVED, ({ requestId }) => {
      this.setState({ latestRequest: this.props.API.getCapturedRequest(requestId) });
    });
  }
      ...
      <div style={ style }
           ...
           onClick={ this.props.onEdit }>
        {
          ...
          <div>
            ...
            <div style={ actionsStyle }>{ this.state.latestRequest.response ? 'Edit' : 'Pending' }</div>
          </div>
        }
      </div>
    );
