...
export class MockedRequests extends React.Component {
  ...
  _requestUrlFor(request) {
    if (request.id === this.state.editingRequestId) {
      return <input type="text"
        autoFocus
        value={ this.state.editedRequestUrl }
        onChange={ this._handleRequestUrlChange.bind(this) }
        onBlur={ this._submitRequestUrl.bind(this, request) }/>
    } else {
      return <Url url={ request.url } pencilEdit={ this._editRequestUrl.bind(this, request) }/>
    }
  }

  _editRequestUrl(request) {
    this.setState({
      editingRequestId: request.id,
      editedRequestUrl: request.url
    });
  }

  _handleRequestUrlChange(event) {
    this.setState({ editedRequestUrl: event.target.value });
  }

  _submitRequestUrl(request) {
    this.setState({
      editingRequestId: null,
      editedRequestUrl: null
    });
    API.updateMockedRequestUrl(request.id, this.state.editedRequestUrl);
  }
