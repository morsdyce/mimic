...
class MockedRequestsSidebar extends React.Component {
  ...
  getMocks() {
    return API.mocks
      ...
      .filter((mock) => this.props.customFilter ? this.props.customFilter(mock) : true)
      .map((mock) => (
        <Mock
          key={ mock.id }
          id={ mock.id }
...
export default DropTarget('mock', groupTarget, collect)(UIStateListener(MockedRequestsSidebar));
