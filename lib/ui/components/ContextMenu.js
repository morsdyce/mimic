...
class ContextMenu extends React.Component {
  ...
  renameSelected = () => {
    const mock = UIState.selectedMocks[0];

    this.closeMenu();
    UIState.update({ sidebarRenameItemId: mock.id });
  };

  cloneSelected = () => {
    UIState.selectedMocks.forEach((mock) => API.duplicateMock(mock.id));

    if (UIState.selectedGroup) {
      API.duplicateGroup(UIState.selectedGroup.id);
    }
  ...
  recaptureMocks = () => {
    API.recaptureMocks(map(UIState.selectedMocks, 'id'), UIState.triggerUpdates.bind(UIState));
  ...
  render() {
    ...
    return (
      <MenuWrapper style={{ top: UIState.mocksSidebarMenu.y, left: UIState.mocksSidebarMenu.x }}
        ...
        <MenuItem onClick={ this.renameSelected } disabled={ UIState.selectedMocks.length > 1 }>
