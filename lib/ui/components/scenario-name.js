...
function mapDNDToProps(connect, monitor) {
  return {
...
export class ScenarioName extends React.Component {
  ...
  _removeScenario(event) {
    event.stopPropagation();
    API.removeScenario(this.props.scenario.id);
  }

  _duplicateScenario(event) {
    event.stopPropagation();
  ...
  _toggleCurrentScenario() {
    API.setCurrentScenario(
      API.currentScenario === this.props.scenario.id ? null : this.props.scenario.id
    );
  }
  ...
  render() {
    let classNames = cx(
      'scenario-name',
      this.props.isOver ? 'drag-active' : null
    );

    return this.props.dropTarget(
      <li className={ classNames } onClick={ this._selectScenario.bind(this) }>

        <a className={ this._rowClassName() }>
          <div>
            <ActiveIndicator active={ API.currentScenario === this.props.scenario.id }
                             onToggle={ () => this._toggleCurrentScenario() }/>
