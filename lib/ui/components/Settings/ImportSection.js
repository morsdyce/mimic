...
class ImportSection extends React.Component {
  ...
  selectFile = (event) => {
    ...
    const mode = this.clearCheckbox.checked ? 'clear' : 'append';
    const options = { mode };
    if (file.name === "swagger.json") {
      options.external_import = "swagger";
    }

    reader.onload = function handleFileUpload() {
      API.import(this.result, options).then((status) => {
        if (status.success) {
          UIState.setViewMode('mocks');
        } else {
          SettingsState.setError(status.error);
      ...
      <SettingsSectionContainer>
        ...
        <SettingsBlock>
          <InputControl
            ...
            defaultValue="https://..."
            inputRef={ (node) => this.urlInput = node }
        ...
        <SettingsBlock>
          <Div>
            <label htmlFor="override">
              <InputControl
                inputRef={ (node) => this.clearCheckbox = node }
                id="override"
                type="radio"
                defaultChecked
        ...
        <SettingsBlock>
          ...
          <InputControl
            style={ SettingsHiddenFileInput }
            inputRef={ (node) => this.importFileInput = node }
            type="file"
            name="file"
