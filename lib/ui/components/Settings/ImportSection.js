...
class ImportSection extends React.Component {
  ...
  importURL = () => {
    ...
    API.importUrl(this.urlInput.value, { mode }, (status) => {
      if (status.success) {
        SettingsState.setImportSuccess();
      } else {
  ...
  render() {
    return (
      <SettingsSectionContainer>
        ...
        <SettingsBlock>
          { this.props.importSuccess && <ImportSuccessMessage>Imported successfully</ImportSuccessMessage> }
          { this.props.importFailed &&
          <ImportErrorMessage>Import failed: { this.props.error }</ImportErrorMessage> }
        </SettingsBlock>
      </SettingsSectionContainer>
    );
  }
