        ...
        <ScrollableContainer>
          {
            API.capturedRequests
              ...
              .map((request) => {
                const { method, url, params, response: { status, delay, body } } = request;
                return (
                  <RequestRow
                    key={ request.id }
                    ...
                    status={ status }
                    mock={ mock }>
                    <Cell style={{ width: 80 }} status={ status }>{ method }</Cell>
                    <Cell style={{ width: 200 }}
                          dangerouslySetInnerHTML={{
                            __html: getHighlightedText(url, UIState.requestsQuery)
                          }}
                          status={ status } />
                    <Cell style={{ width: 160 }} status={ status }>{ params }</Cell>
                    <Cell style={{ width: 40 }}  status={ status }>{ status }</Cell>
                    <Cell style={{ width: 40 }}  status={ status }>{ delay }</Cell>
