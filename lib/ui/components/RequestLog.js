  ...
  render() {
    return (
      <Frame controls={ this.controls() }>
        <HeaderRow>
          <Cell style={{ width: UIState.logColumns.method }}>Method</Cell>
          <Cell style={{ width: UIState.logColumns.url }}>URL</Cell>
          <Cell style={{ width: UIState.logColumns.params }}>Request Params</Cell>
          <Cell style={{ width: UIState.logColumns.status }}>Status</Cell>
          <Cell style={{ width: UIState.logColumns.delay }}>Delay</Cell>
              ...
              .map((request) => {
                ...
                return (
                  <RequestRow
                    ...
                    status={ status }
                    mock={ mock }>
                    <Cell style={{ width: UIState.logColumns.method }} status={ status }>{ method }</Cell>
                    <Cell style={{ width: UIState.logColumns.url }}
                          dangerouslySetInnerHTML={{
                            __html: getHighlightedText(url, UIState.requestsQuery)
                          }}
                          status={ status } />
                    <Cell style={{ width: UIState.logColumns.params }} status={ status }>{ params }</Cell>
                    <Cell style={{ width: UIState.logColumns.status }}  status={ status }>{ status }</Cell>
                    <Cell style={{ width: UIState.logColumns.delay }}  status={ status }>{ delay }</Cell>
                    <Cell style={{ flex: '1' }}  status={ status }>{ body }</Cell>
                  </RequestRow>
                )
              }).reverse()
          }
