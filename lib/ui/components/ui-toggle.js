...
export class UIToggle extends React.Component {
  ...
  componentWillMount() {
    try {
      const uiState = JSON.parse(localStorage.getItem('__mimicui__'));

      this._setPosition(uiState.togglePosition);
    } catch (err) {
      // unable to persist toggle position
    }
  ...
  _setPosition(position = 'bottom-right') {
    ...
    localStorage.setItem('__mimicui__', JSON.stringify({ 'togglePosition': position }));
  }
