...
export class ScenarioOverview extends Component {
  ...
  _renderMasterToggle() {
    const toggleAllRequests = () => {
      const nextState = this.state.masterToggle;
      this.props.mockedRequests.forEach( request => {
        if( (nextState && !request.active) || (!nextState && request.active ) ) {
      ...
      <div className="row">
          <div className="col-xs-2 col-xs-offset-10">
            <Toggle
              ...
              onToggle={ () => {
                this.setState( { masterToggle : !this.state.masterToggle }, toggleAllRequests );
              } }/>
          </div>
      </div>
    )
