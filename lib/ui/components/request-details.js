...
import { Tabs, Tab, TextField, RaisedButton, FloatingActionButton } from 'material-ui';
...
class RequestDetails extends Component {
  ...
  _handleSave() {
    if (this.props.mock && this.state.selectedScenario === this.state.originalScenario) {
      ...
      this.props.mockRequest(this.state.selectedScenario, {
        ...
        response: {
          delay: _get(this.props, 'response.delay', 0),
          status: _get(this.props, 'response.status', 200),
          body: this.state.responseBody
        }
      });
    }
  ...
  _renderHeaders() {
    return map(this.state.headers, (value, key, collection, index) => {
      return (
        <div className="row" key={ index }>
          <div className="col-xs-3">
            <TextField
              hintText="Header"
              value={ key }
              onChange={ this._headersChange.bind(this, key, 'key') }
            />
          </div>
          ...
          <div className="col-xs">
            <TextField
              ...
              style={{ width: '80%' }}
              value={ value }
              onChange={ this._headersChange.bind(this, key, 'value') }/>
          </div>
        </div>
      );
    });
