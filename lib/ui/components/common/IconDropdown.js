...
export class IconDropdown extends React.Component {
  ...
  renderOptions = () => {
    ...
    const options = this.props.options.map((option) => {
      ...
      <Option
        ...
        isPadded={ this.props.value && this.props.value !== option.value}>
        { this.props.value === option.value && <SelectedIcon src="check"/> }
        { option.label }
      </Option>
    );
    });
  ...
  render() {
    const optionSelected = this.props.options.slice(1).some((option) => option.value === this.props.value);
