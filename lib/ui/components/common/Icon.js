...
import collapseAll from 'ui/icons/collapse-all.png';
import expandAll from 'ui/icons/expand-all.png';
import filter from 'ui/icons/filter.png';
import undo from 'ui/icons/undo.png';
...
const Icon = ({ src, style, onClick, className }) => {
  if (['settings'].indexOf(src) !== -1) {
    return (
      <StyledDiv style={ style } dangerouslySetInnerHTML={{ __html: ICONS[src] }} className={ className }/>
    );
  } else {
    return (
